...
#   - Un préfixe optionnel du type "BOOK 1 – " ou "BOOK 1 - "
#   - Des variantes de type "CS-E", "CS-APU", "CS-P", etc.
#   - Un titre optionnel après le numéro
#   - Des numéros comme "25.613", "25.613A", "25.613(a)", "25.613(a)(1)" etc.
# ----------------------------------------------------------------------
HEADER_RE = re.compile(
    r"""^
        \s*
        (?:BOOK\s+\d+\s*[-–—]\s+)?          # préfixe optionnel "BOOK 1 -/–/— "
        ((CS|AMC|GM)(?:-[A-Z0-9]+)?)        # groupe 1: kind complet (CS, AMC, GM, CS-E, CS-APU...)
        \s+
        ([0-9]+(?:\.[0-9A-Za-z]+)*          # groupe 3: numéro principal, ex: 25.613, 25.613A
           (?:\([0-9A-Za-z]+\))*            # éventuels sous-paragraphes entre parenthèses
        )
...
def split_easa_sections(text: str) -> List[Dict]:
    ...
    for raw_line in lines:
        ...
        if m:
            ...
            if current_kind and current_number is not None:
